...
/**
 * Required parameters to process the turnover.
 */
...
export class TurnoverService {
	 ...
	 * @returns the resulting turnover or -1 if the calculation is not possible
	 */
	public turnover (year: number, external = false) {

		const turnoverParameters = new TurnoverParameters();

		this.staffListService.allStaff
			.filter(staff => (!external) ? (!staff.external) : true )
			.forEach( staff => this.takeInAccount(staff, year, turnoverParameters));

		if (turnoverParameters.total === 0) {
			return TurnoverService.NO_DATA_AVAILABLE;
		}

		// turnover = (resignation count + (arrival count / 2)) / number of workers at the 1st january
		const turnover = Math.round( ((turnoverParameters.resignation + turnoverParameters.arrival) / 2) * 100 / turnoverParameters.total );
		if (traceOn()) {
			console.log ('Calculation of the turnover for year %d : ( ((%d + %d) / 2) * 100 / %d) = %d.', year, 
				turnoverParameters.resignation, turnoverParameters.arrival, turnoverParameters.total, 
				turnover);
		}
		return turnover;
	}

	private takeInAccount(staff: Collaborator, year: number, turnoverParameters: TurnoverParameters) {
		...
		if (year === new Date(Date.now()).getFullYear()) {
			...
			if ( (!staff.active) && (lastExperience.lastCommit.getFullYear() >= year) && (lastExperience.lastCommit.getFullYear() < (year + 1)))   {
				turnoverParameters.resignation++
			}
		} else {
