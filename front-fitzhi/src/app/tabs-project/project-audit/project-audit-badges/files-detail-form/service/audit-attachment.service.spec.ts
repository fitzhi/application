...
describe('AuditAttachmentService', () => {
	...
	beforeEach(() => {
		...
		projectService.project.audit[0].attachmentList
			.push(new AttachmentFile(0, 'filename.pdf', 0, 'my label'));
	...
	it('should be created', done => {
		...
		auditAttachmentService.attachmentFiles$.subscribe({
			next: attachmentFiles => {
				...
				expect(auditAttachmentService.attachmentFiles.get(3).length).toBe(1);
				done();
			}
		})
	});
