...
describe('TreemapSkillsComponent container', () => {
	...
	beforeEach(() => {
		localStorage.setItem('helpHeight', '0');
	...
	it('should hide the help pane if the cancellation of this feature is configured in the local storage.', done => {
		loadChart();
		fixture.detectChanges();
		...
		expect(document.getElementById('btHelp').classList[2]).toBe('btn-outline-secondary');
