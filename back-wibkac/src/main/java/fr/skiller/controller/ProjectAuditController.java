 ...
 * 
 * <p>
 * Controller in charge the collection of {@link AuditProject} declared in the project.
 * </p>
 *
...
public class ProjectAuditController {
		...
		if (log.isDebugEnabled()) {
			log.debug(String.format(
				"POST command on /project/audit/addTopic for project.id %d and topic.id %d", 
						param.getIdProject(), param.getAuditProject().getIdTopic()));
		}
		
		try {
			projectAuditHandler.addTopic(param.getIdProject(), param.getAuditProject().getIdTopic());
	...
	/**
	 * load and return an {@link AuditProject}
	...
	public ResponseEntity<AuditProject> getTopicAudit(
		...
		try {
			AuditProject auditProject = projectAuditHandler.getTopic(idProject, idTopic);
	...
	public ResponseEntity<Boolean> removeTopic(@RequestBody BodyParamAuditEntry param) {
		...
		try {
			projectAuditHandler.removeTopic(param.getIdProject(), param.getAuditProject().getIdTopic(), false);
