...
public class FileSystemStorageService implements StorageService {
    ...
    public void init() {
        try {
            ...
            log.info("directory " + rootLocation);
        }
        catch (IOException e) {
	...
	public String readFileTXT(final String fileName) throws IOException {
      	if (log.isDebugEnabled()) {
    		log.debug(String.format("readFileTXT (%s)", this.rootLocation.resolve(fileName)));
    	}
		...
		br.close();
    	if (log.isDebugEnabled()) {
    		log.debug(String.format("readFileTXT returns %d characters.", sb.toString().length()));
    	}
		return sb.toString();
	}
	...
	public String readFileDOCX(final String fileName) throws IOException {
      	if (log.isDebugEnabled()) {
    		log.debug(String.format("readFileDOCX (%s)", this.rootLocation.resolve(fileName)));
    	}
		...
		we.close();
    	if (log.isDebugEnabled()) {
    		log.debug(String.format("readFileDOCX returns %s characters.", content.length()));
    	}
		return content;
	}
    ...
    public String readFilePDF(final String fileName) throws IOException {
      	if (log.isDebugEnabled()) {
    		log.debug(String.format("readFilePDF (%s)", this.rootLocation.resolve(fileName)));
    	}
		...
		reader.close();
    	if (log.isDebugEnabled()) {
    		log.debug(String.format("readFilePDF returns %d characters.", sb.toString().length()));
    	}
		return sb.toString();
	}
