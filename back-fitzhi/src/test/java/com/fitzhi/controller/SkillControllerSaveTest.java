...
public class SkillControllerSaveTest {
	...
	public void saveUnknownSkill() throws Exception {
		...
		this.mvc.perform(post(SKILL_SAVE)
	...
	public void saveNewSkill() throws Exception {
		...
		System.out.println(gson.toJson(skill));
		MvcResult result = this.mvc.perform(post(SKILL_SAVE)
			.contentType(MediaType.APPLICATION_JSON_UTF8)
			.content(gson.toJson(skill)))
		...
		Assert.assertEquals(SkillDetectorType.FILENAME_DETECTOR_TYPE, retSkill.getDetectionPattern().getDetectionType());
	...
	public void saveSkillUpdateDetectionPattern() throws Exception {
		...
		result = this.mvc.perform(post(SKILL_SAVE)
				.contentType(MediaType.APPLICATION_JSON_UTF8)
